...
for (let iDID = 0; iDID < unitsArray.length; iDID++) { // 对每一个sensor做循环 iDID:传感器ID循环
    ...
    c('  ---- Phase II： computing occupancy  records for this DID (BOUNDARY ADDED) : ' + recordsofSensor.length + ' lists')
    ...
    for (let iRec = 1; iRec < recordsofSensor.length; iRec++) { // start from second record
        ...
        // 得到格子差和秒数零头
        c('  --- (' + iRec + ') ' + t1.toLocaleString() + ' -  ' + t2.toLocaleString())

        diffofGrid = Math.floor((t2m - t1m - 1) / (grid * 60 * 1000)) // 两次数据之间整格子差
        t1toNextgrid = 60 * grid - t1s.getSeconds() - ((((t1s.getMinutes() % grid) === 0) && (t1s.getSeconds() === 0)) ? grid : (t1s.getMinutes() % grid)) * 60 // t1到下一个格子的秒数.例如 16:14:06,格子为30 min,  则 =1800-6-840 
        ...
        if (recordsofSensor[iRec - 1].value === 'in') { // 如果前一个是in,那么后面的时间段应该100%占用
            ...
            if (occuArray.length >= 1) {
                for (let iOccu = occuArray.length - 1; iOccu >= Math.max(occuArray.length - 5, 0); iOccu--) { // 检查是否存在这个十分钟纪录,回溯若干个记录
                    if (occuArray[iOccu].timeStamp === t0.toLocaleString()) {
                        c('    -- 头部记录存在，不改变原值 ' + t0.toLocaleTimeString() + '   ' + JSON.stringify(occuArray[iOccu]))
                        _RecordExist = true
                        break
                    }
                }
