...
for (let lc = 0; lc < locationIds.length; lc++) {
    loggers[lc] = childProcess.fork('get_data_worker_beta.js', [locationIds[lc], startDate, endDate, EUorUU])
        // console.log("child process entered")
    loggers[lc].on('exit', function() {
        Converters[lc] = childProcess.fork('cal_data_worker_beta.js', [locationIds[lc], startDate, endDate, EUorUU, interval])
    })
}
